...
function buildLobbyScreen() {
    ...
    // Setup 3D character preview after a short delay to ensure DOM is ready
    setTimeout(() => {
        setupLobbyCharacter();
    }, 100);
    ...
    const armoryBtn = document.getElementById('lobbyArmoryBtn');
    const lockerBtn = document.getElementById('lobbyLockerBtn');
    const playBtn = document.getElementById('playBtn');
    
    if (settingsBtn) settingsBtn.addEventListener('click', () => openSettings());
    if (armoryBtn) armoryBtn.addEventListener('click', () => openArmory());
    if (lockerBtn) lockerBtn.addEventListener('click', () => openLocker());
    if (playBtn) {
...
function setupLobbyCharacter() {
    ...
    if (!container) {
        console.warn('Lobby character container not found');
        return;
    }
    ...
    lobbyCamera = new THREE.PerspectiveCamera(50, 1, 0.1, 1000);
    lobbyCamera.position.set(0, 1.2, 3.2);
    ...
    function animateLobby() {
        ...
        if (lobbyCharacter) {
            ...
            lobbyCharacter.position.y = Math.sin(time * 1.5) * 0.03;
        }
...
export function showLobby() {
    ...
    // Re-render lobby character
    setTimeout(() => {
        if (lobbyRenderer && lobbyCamera && lobbyScene) {
            lobbyRenderer.render(lobbyScene, lobbyCamera);
        }
    }, 50);
}
