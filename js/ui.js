...
export function createUI() {
    // Health and Shield bars (bottom left)
    ...
    // Weapon slots with custom SVG icons
    ...
    weaponContainer.innerHTML = `
        <div class="weapon-slots" id="weaponSlots">
            <div class="weapon-slot active" data-weapon="pistol" data-key="1">
                ...
                <div class="weapon-slot-icon">${getPistolIcon()}</div>
            ...
            <div class="weapon-slot" data-weapon="assault" data-key="2">
                ...
                <div class="weapon-slot-icon">${getAssaultIcon()}</div>
            ...
            <div class="weapon-slot" data-weapon="sniper" data-key="3">
                ...
                <div class="weapon-slot-icon">${getSniperIcon()}</div>
            ...
            <div class="weapon-slot" data-weapon="shotgun" data-key="4">
                ...
                <div class="weapon-slot-icon">${getShotgunIcon()}</div>
    ...
    // Flashlight UI indicator
    const flashlightIndicator = document.createElement('div');
    flashlightIndicator.id = 'flashlightIndicator';
    flashlightIndicator.className = 'flashlight-indicator hidden';
    flashlightIndicator.innerHTML = '🔦 FLASHLIGHT ON';
    document.body.appendChild(flashlightIndicator);

    // Cave ambience indicator (shows when in dark cave)
    const caveIndicator = document.createElement('div');
    caveIndicator.id = 'caveIndicator';
    caveIndicator.className = 'cave-indicator hidden';
    caveIndicator.innerHTML = '🦇 DARK CAVE 🦇';
    document.body.appendChild(caveIndicator);

    // Coins display
    const coinsDisplay = document.createElement('div');
    coinsDisplay.className = 'coins-display';
    coinsDisplay.innerHTML = `💰 <span id="coinVal">0</span>`;
...
// ─── GUN ICONS (Custom SVG, Fortnite/1v1.lol style) ───────────────────────────
function getPistolIcon() {
    return `<svg class="weapon-svg" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
        <rect x="20" y="28" width="28" height="12" rx="2" fill="#4a4a52" stroke="#2a2a32" stroke-width="2"/>
        <rect x="40" y="26" width="12" height="16" rx="1" fill="#3a3a42" stroke="#2a2a32" stroke-width="2"/>
        <rect x="16" y="30" width="6" height="8" rx="1" fill="#5a5a62" stroke="#2a2a32" stroke-width="1.5"/>
        <circle cx="48" cy="32" r="2" fill="#888" stroke="#2a2a32" stroke-width="1"/>
        <rect x="22" y="34" width="18" height="3" fill="#2a2a32"/>
    </svg>`;
}

function getAssaultIcon() {
    return `<svg class="weapon-svg" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
        <rect x="15" y="26" width="35" height="14" rx="2" fill="#4a4a52" stroke="#2a2a32" stroke-width="2"/>
        <rect x="42" y="22" width="14" height="22" rx="1" fill="#3a3a42" stroke="#2a2a32" stroke-width="2"/>
        <rect x="12" y="28" width="6" height="10" rx="1" fill="#5a5a62" stroke="#2a2a32" stroke-width="1.5"/>
        <rect x="18" y="36" width="25" height="3" fill="#2a2a32"/>
        <rect x="28" y="38" width="12" height="8" rx="1" fill="#3a3a42" stroke="#2a2a32" stroke-width="1"/>
        <rect x="50" cy="32" width="3" height="6" fill="#555"/>
    </svg>`;
}

function getSniperIcon() {
    return `<svg class="weapon-svg" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
        <rect x="12" y="28" width="40" height="10" rx="2" fill="#4a5a4a" stroke="#2a3a2a" stroke-width="2"/>
        <rect x="44" y="22" width="16" height="22" rx="2" fill="#3a4a3a" stroke="#2a3a2a" stroke-width="2"/>
        <rect x="8" y="30" width="6" height="6" rx="1" fill="#5a6a5a" stroke="#2a3a2a" stroke-width="1.5"/>
        <circle cx="36" cy="29" r="4" fill="#88aaff" stroke="#2a3a2a" stroke-width="1.5"/>
        <circle cx="36" cy="29" r="1.5" fill="#2266aa"/>
        <rect x="14" y="34" width="22" height="2" fill="#2a3a2a"/>
    </svg>`;
}

function getShotgunIcon() {
    return `<svg class="weapon-svg" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
        <rect x="18" y="28" width="30" height="12" rx="2" fill="#5a4a3a" stroke="#3a2a1a" stroke-width="2"/>
        <rect x="40" y="24" width="14" height="20" rx="1" fill="#4a3a2a" stroke="#3a2a1a" stroke-width="2"/>
        <rect x="14" y="30" width="6" height="8" rx="1" fill="#6a5a4a" stroke="#3a2a1a" stroke-width="1.5"/>
        <rect x="20" y="36" width="20" height="3" fill="#3a2a1a"/>
        <circle cx="20" cy="32" r="3" fill="#8a6a4a" stroke="#3a2a1a" stroke-width="1"/>
        <circle cx="28" cy="32" r="3" fill="#8a6a4a" stroke="#3a2a1a" stroke-width="1"/>
    </svg>`;
}
...
export function updateCaveIndicatorUI(inCave) {
    ...
    if (indicator) {
        if (inCave) indicator.classList.remove('hidden');
        else indicator.classList.add('hidden');
    }
}
...
export function updateTimeOfDayUI(timeOfDay, isNight) {
    ...
    if (!timeDiv) {
        ...
        timeDiv.style.cssText = 'position:fixed; top:80px; left:20px; background:rgba(0,0,0,0.6); backdrop-filter:blur(8px); border-radius:20px; padding:6px 16px; font-size:12px; font-weight:bold; color:#ffdd88; z-index:20; pointer-events:none;';
...
function buildLobbyScreen() {
    ...
    lobby.innerHTML = `
        <div class="lobby-bg-particles"></div>
        <h1 class="lobby-title">PATAPIM ARENA</h1>
        <p class="lobby-subtitle">SELECT GAME MODE</p>
        <div class="lobby-modes">
            <div class="mode-card" data-mode="zombies"><span class="mode-tag new">HOT</span><span class="mode-icon zombies">🧟</span><span class="mode-name">Zombies</span><span class="mode-desc">Survive endless waves</span></div>
            <div class="mode-card" data-mode="practice"><span class="mode-icon practice">🎯</span><span class="mode-name">Practice</span><span class="mode-desc">Train your aim</span></div>
            <div class="mode-card disabled" data-mode="battle-royale"><span class="mode-tag coming-soon">SOON</span><span class="mode-icon battle-royale">🏆</span><span class="mode-name">Battle Royale</span><span class="mode-desc">Last one standing</span></div>
        </div>
        <div class="lobby-footer"><button class="lobby-btn" id="lobbySettings">Settings</button><button class="lobby-btn" id="lobbyArmory">Armory</button><button class="lobby-btn" id="lobbyLocker">Locker</button></div>
    `;
...
export function showLobby() { const lobby = document.getElementById('lobbyScreen'); if (lobby) lobby.classList.remove('hidden'); }
export function hideLobby() { const lobby = document.getElementById('lobbyScreen'); if (lobby) lobby.classList.add('hidden'); }
...
function buildSettingsMenu() {
    ...
    overlay.innerHTML = `<div class="menu-window settings-window"><div class="menu-title">SETTINGS</div><div class="settings-tabs"><button class="tab-btn active" data-tab="controls">Controls</button><button class="tab-btn" data-tab="keybinds">Keybinds</button><button class="tab-btn" data-tab="graphics">Graphics</button><button class="tab-btn" data-tab="audio">Audio</button></div>
        <div class="tab-content" id="tab-controls"><div class="setting-row"><label>Mouse Sensitivity</label><input type="range" min="5" max="200" value="100" id="sensitivitySlider"><span id="sensitivityVal">1.0x</span></div>
        ...
        <div class="setting-row"><label>Invert Y</label><button class="toggle-btn" id="invertYBtn">OFF</button></div></div>
        ...
        <div class="tab-content hidden" id="tab-graphics"><div class="setting-row"><label>Shadows</label><div class="btn-group"><button class="opt-btn" data-val="off">Off</button><button class="opt-btn" data-val="low">Low</button><button class="opt-btn active" data-val="high">High</button></div></div>
        <div class="setting-row"><label>FPS Counter</label><button class="toggle-btn" id="fpsBtn">OFF</button></div>
        <div class="setting-row"><label>Crosshair Color</label><input type="color" id="crosshairColorPicker" value="#ffffff"></div></div>
        <div class="tab-content hidden" id="tab-audio"><div class="setting-row"><label>Master Volume</label><input type="range" min="0" max="100" value="80" id="masterVolSlider"><span id="masterVolVal">80</span></div>
        <div class="setting-row"><label>SFX Volume</label><input type="range" min="0" max="100" value="100" id="sfxVolSlider"><span id="sfxVolVal">100</span></div></div>
...
function populateKeybindsGrid() {
    ...
    if (!grid) return;
    const actions = [['forward','Forward'],['back','Back'],['left','Left'],['right','Right'],['jump','Jump'],['sprint','Sprint'],['reload','Reload'],['weapon1','Weapon 1'],['weapon2','Weapon 2'],['weapon3','Weapon 3'],['weapon4','Weapon 4'],['buildToggle','Build Mode']];
    grid.innerHTML = '';
...
function buildArmoryMenu() {
    ...
    overlay.className = 'menu-overlay hidden';
    overlay.innerHTML = `<div class="menu-window armory-window"><div class="menu-title">ARMORY</div><div class="armory-layout"><div class="weapon-list" id="armoryWeaponList"></div><div class="upgrade-panel" id="armoryUpgradePanel"><div class="upgrade-placeholder">Select a weapon to view upgrades</div></div></div><div class="menu-footer"><button class="menu-btn" id="closeArmory">Close</button></div></div>`;
    document.body.appendChild(overlay);
...
function showWeaponUpgrades(wid) {
    ...
    Object.entries(upgrades).forEach(([key, upg]) => {
        ...
        const tierBars = upg.levels.map((val, i) => `<div class="tier-bar ${i <= currentTier ? 'active' : ''}" title="Tier ${i}: ${val}${upg.lower ? 's' : ''}">${val}${upg.lower ? 's' : ''}</div>`).join('');
        const nextCost = currentTier < maxTier ? upg.costs[currentTier] : null;
        const upgradeBtn = nextCost !== null ? `<button class="upgrade-btn" data-weapon="${wid}" data-key="${key}">Upgrade - 💰${nextCost}</button>` : `<button class="upgrade-btn maxed" disabled>MAX</button>`;
        const block = document.createElement('div');
        block.className = 'upgrade-block';
        block.innerHTML = `<div class="upgrade-header"><span class="upgrade-label">${upg.label}</span><div class="tier-bars">${tierBars}</div></div>${upgradeBtn}`;
    ...
    panel.querySelectorAll('.upgrade-btn:not(.maxed)').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (window.getCoins && window.getCoins() < cost) { btn.innerText = 'Not enough coins!'; setTimeout(() => showWeaponUpgrades(w), 1000); return; }
...
function buildLockerMenu() {
    ...
    overlay.className = 'menu-overlay hidden';
    overlay.innerHTML = `<div class="menu-window locker-window"><div class="menu-title">LOCKER</div><div class="locker-layout"><div class="color-options"><div class="color-row"><label>Shirt Color</label><input type="color" id="shirtColorPicker" value="${COSMETICS.bodyColor}"></div><div class="color-row"><label>Pants Color</label><input type="color" id="pantsColorPicker" value="${COSMETICS.accentColor}"></div><div class="color-row"><label>Skin Tone</label><input type="color" id="skinColorPicker" value="${COSMETICS.headColor}"></div></div><div class="locker-preview"><div class="preview-label">Preview updates in-game</div></div></div><div class="menu-footer"><button class="menu-btn primary" id="applyLocker">Apply</button><button class="menu-btn" id="closeLocker">Cancel</button></div></div>`;
    document.body.appendChild(overlay);
    document.getElementById('applyLocker').addEventListener('click', () => { COSMETICS.bodyColor = document.getElementById('shirtColorPicker').value; COSMETICS.accentColor = document.getElementById('pantsColorPicker').value; COSMETICS.headColor = document.getElementById('skinColorPicker').value; if (window.applyCosmetics) window.applyCosmetics(); closeLocker(); });
