...
// Articulated zombie - boxy 1v1.lol-style anatomy but zombified
export function createZombieEnemy(x, z) {
    ...
    // Tear/stripe of skin showing through
    ...
    // Head with hollow eye sockets (no detailed face)
    ...
    headGroup.rotation.x = 0.2;  // hunched/looking forward
    group.add(headGroup);

    // Articulated arm (reaching forward by default)
    ...
    leftArm.rotation.x = -1.0;  // reaching forward
...
// Update zombie: move toward player, animate, apply damage
export function updateEnemies(enemies, dt, playerPos, onDamagePlayer, getGroundHeight) {
    for (let i = enemies.length - 1; i >= 0; i--) {
        ...
        if (dist > 1.2) {
            ...
            // Attack
            if (ud.damageCD <= 0) {
                onDamagePlayer(CONFIG.enemies.zombie.damageToPlayer);
                ud.damageCD = 1.0;
            }
            ud.walkCycle += dt * 8;
        }
        ...
        // Arms reach forward with slight wobble
        ...
        if (ud.torso) {
            ...
            ud.torso.rotation.x = 0.15;  // hunched
        }
...
export function spawnWave(scene, enemies, waveNumber, mapBounds = 80) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        const radius = 25 + Math.random() * mapBounds * 0.4;
        const x = Math.cos(angle) * radius;
        const z = Math.sin(angle) * radius;
        const z_clamped = Math.max(-mapBounds, Math.min(mapBounds, z));
        const x_clamped = Math.max(-mapBounds, Math.min(mapBounds, x));
        const zombie = createZombieEnemy(x_clamped, z_clamped);
...
// Practice mode dummies (just standing targets)
export function createDummy(x, z) {
    const group = new THREE.Group();
    ...
    // Stripes
    const stripe = new THREE.Mesh(new THREE.BoxGeometry(0.71, 0.1, 0.46), accent);
