...
// Play gun sound using Web Audio
...
export function switchWeapon(id) {
    ...
    if (CONFIG.weapons[id]) {
        ...
        if (window.updateWeaponUI) {
            window.updateWeaponUI(currentWeapon, ammoLeft, false);
        }
        if (window.updateGunVisuals) {
            window.updateGunVisuals(id);
        }
    }
}
...
export function reloadWeapon() {
    ...
    reloading = true;
    if (window.updateWeaponUI) {
        window.updateWeaponUI(currentWeapon, ammoLeft, true);
    }
...
export function shootWeapon(raycaster, camera, scene, enemies, onHit, bulletTrails) {
    ...
    // Direction and raycast
    const dir = new THREE.Vector3(0, 0, -1).applyQuaternion(camera.quaternion);
    const origin = camera.position.clone();
    raycaster.set(origin, dir);
    const intersects = raycaster.intersectObjects(enemies, true);
    
    // Bullet trail (yellow/gold)
    ...
    if (intersects.length) {
        ...
        while (obj && !obj.userData.isEnemy) {
            obj = obj.parent;
        }
        if (obj && obj.userData.health) {
            let dmg = wp.damage;
            if (currentWeapon === 'shotgun') {
                dmg = wp.damage * wp.pellets;
            }
            obj.userData.health -= dmg;
            
            if (onHit) {
                onHit(obj.userData.health <= 0);
            }
            ...
            if (obj.userData.health <= 0 && obj.parent) {
                ...
                if (bar) {
                    bar.scale.x = Math.max(0, obj.userData.health / CONFIG.enemies.blob.health);
                }
            }
        }
    }
